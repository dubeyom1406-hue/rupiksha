    ...
    const doFetch = async () => {
        ...
        try {
            ...
            if (data.success && data.bill) {
                ...
                setStatus({
                    type: 'error',
                    message: matchedErr ? `⚠️ ${VENUS_ERRORS[matchedErr]}` : (data.message || '⚠️ Bill Fetch Failed. Please check consumer number and try again.')
                });
            }
        } catch (err) {
            setStatus({
    ...
    const payBill = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setStatus({
                    type: 'error',
                    message: matchedErr ? `⚠️ ${VENUS_ERRORS[matchedErr] || matchedErr}` : (data.message || '⚠️ Payment Failed. Please check balance and try again.')
                });
            }
        } catch (err) {
            setStatus({
                                    ...
                                    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                                        <div>
                                            ...
                                            <div className="relative">
                                                ...
                                                <input
                                                    value={consumerNo}
                                                    onChange={e => { setConsumerNo(e.target.value.replace(/\D/g, '')); setFetchedBill(null); setStep('form'); }}
